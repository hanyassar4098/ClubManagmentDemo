...
export class UserInfoComponent implements OnInit {
  ...
  private loadCurrentUserData() {
    ...
    if (this.canViewAllRoles) {
      this.accountService.getUserAndRoles().subscribe(results => this.onCurrentUserDataLoadSuccessful(results[0], results[1],results[2]), error => this.onCurrentUserDataLoadFailed(error));
    } else {
      this.accountService.getUser().subscribe(results => this.onCurrentUserDataLoadSuccessful(results[0], results[0].roles.map(x => new Role(x) ) , results[1]), error => this.onCurrentUserDataLoadFailed(error));
    }
  }


  private onCurrentUserDataLoadSuccessful(user: User, roles: Role[],services: Service[]) {
    debugger;
  ...
  getServiceByName(name: string) {
    return this.allServices.find((r) => r.name === name);
  }
