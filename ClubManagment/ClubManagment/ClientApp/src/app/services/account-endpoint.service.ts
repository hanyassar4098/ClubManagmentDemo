...
export class AccountEndpoint extends EndpointBase {
  ...
  get servicesUrl() { return this.configurations.baseUrl + '/api/account/services'; }
  ...
  getNewServiceEndpoint<T>(userObject: any): Observable<T> {
    debugger;

    return this.http.post<T>(this.servicesUrl, JSON.stringify(userObject), this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.getNewServiceEndpoint(userObject));
      }));
  }
  ...
  getServicesEndpoint<T>(page?: number, pageSize?: number): Observable<T> {
    const endpointUrl = page && pageSize ? `${this.servicesUrl}/${page}/${pageSize}` : this.servicesUrl;

    return this.http.get<T>(endpointUrl, this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.getServicesEndpoint(page, pageSize));
      }));
  }
