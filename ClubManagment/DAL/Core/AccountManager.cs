...
namespace DAL.Core
{
    public class AccountManager : IAccountManager
    {
        ...
        public async Task<(ApplicationUser User, string[] Roles, string[] Services)?> GetUserAndRolesAsync(string userId)
        {
            var user = await _context.Users
                .Include(u => u.Roles)
                .Include(u => u.Subscribtion)
                .Where(u => u.Id == userId)
            ...
            var userServiceIds = user.Subscribtion.Select(r =>r.ServiceId).ToList();
            ...
            var services = await _context.Service
               .Where(r => userServiceIds.Contains(r.Id))
               .Select(r => r.Name)
               .ToArrayAsync();

            return (user, roles, services);
        }


        public async Task<List<(ApplicationUser User, string[] Roles, string[] Services, decimal Totalcost)>> GetUsersAndRolesAsync(int page, int pageSize)
        {
            ...
            var userServiceIds = users.SelectMany(u => u.Subscribtion.Select(r => r.ServiceId)).ToList();
            ...
            return users
                .Select(u => (u, roles.Where(r => u.Roles.Select(ur => ur.RoleId).Contains(r.Id)).Select(r => r.Name).ToArray(), Services.Where(r => u.Subscribtion.Select(ur => ur.ServiceId).Contains(r.Id)).Select(r => r.Name).ToArray(), Services.Where(r => u.Subscribtion.Select(ur => ur.ServiceId).Contains(r.Id)).Select(r => r.Price).Sum()))
                .ToList();
        }


        public async Task<(bool Succeeded, string[] Errors)> CreateUserAsync(ApplicationUser user, IEnumerable<string> roles, IEnumerable<string> services, string password)
        {
            ...
            try
            {
                if (services != null)
                {
                    var sresult = await this._unitOfWork.Subscribtions.SubscribtionsAsync(user, services.Distinct());
                }
        ...
        {
            return await UpdateUserAsync(user, null,null);
        }


        public async Task<(bool Succeeded, string[] Errors)> UpdateUserAsync(ApplicationUser user, IEnumerable<string> roles, IEnumerable<string> services)
        {
            ...
            if (services != null)
            {
                var userServices = await this._unitOfWork.Subscribtions.GetServicesAsync(user);

                var servicesToRemove = userServices.Except(services).ToArray();
                var servicesToAdd = services.Except(userServices).Distinct().ToArray();

                if (servicesToRemove.Any())
                {
                    var sresult = await this._unitOfWork.Subscribtions.SubscribtionsdeleteAsync(user, servicesToRemove);
                    if (!sresult)
                        return (false, result.Errors.Select(e => e.Description).ToArray());
                }
                ...
                {                    
                    var sresultAdd = await this._unitOfWork.Subscribtions.SubscribtionsAsync(user, servicesToAdd);
                    if (!sresultAdd)
                        return (false, result.Errors.Select(e => e.Description).ToArray());
                }
            }
        ...
        {
            if (await _context.Subscribtion.Where(o => o.SubscriberId == userId).AnyAsync())
                return false;
